...
export default function Analytics() {
  ...
  const getStatusBadge = (goal) => {
    ...
    if (progress >= 100) return { text: 'Completed', color: 'bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200' };
    if (now > endDate) return { text: 'Expired', color: 'bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-200' };
    return { text: 'Active', color: 'bg-primary/20 text-blue-800' };
  };
    ...
    <DashboardLayout>
      <div className="max-w-7xl mx-auto">
        <div className="flex justify-between items-center mb-8">
          <div>
            <h1 className="text-3xl font-bold text-foreground mb-2">Reading Analytics</h1>
        ...
        >
          <h2 className="text-xl font-bold text-foreground mb-6">Monthly Reading Time</h2>
          <div className="space-y-4">
            {weeklyData.length > 0 ? weeklyData.map((day, index) => (
              <div key={day.day} className="flex items-center gap-4">
                <span className="w-12 text-sm font-medium text-muted-foreground">{day.day}</span>
                <div className="flex-1 bg-muted rounded-full h-8 relative overflow-hidden">
                  {day.minutes > 0 ? (
                    <motion.div
                      ...
                      className="bg-gradient-to-r from-primary to-primary h-full rounded-full flex items-center justify-end pr-3"
                    >
                      <span className="text-white text-sm font-semibold">{day.minutes}m</span>
                    </motion.div>
                  ) : (
                    <div className="flex items-center justify-center h-full">
                      <span className="text-muted-foreground text-xs">No reading time</span>
                    </div>
                  )}
                </div>
                <span className="w-16 text-sm text-muted-foreground">{day.pages} pages</span>
              </div>
            )) : (
              <p className="text-center text-muted-foreground py-8">No reading data available yet. Start reading to see your progress!</p>
            )}
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="space-y-6">
            {goals.length > 0 ? goals.map((goal) => {
              ...
              return (
                <div key={goal.id} className="border border-border rounded-lg p-4 hover:shadow-md transition-shadow">
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        <h3 className="font-semibold text-foreground">{getGoalTypeLabel(goal.goal)}</h3>
                      ...
                      <p className="text-xs text-muted-foreground mt-1">{getDaysRemaining(goal.end_date)}</p>
                    </div>
                    <div className="flex gap-2">
                      <button
                      ...
                      <button
                        ...
                        className="p-1.5 text-red-600 dark:text-red-400 hover:bg-red-50 dark:bg-red-900/20 rounded transition-colors disabled:opacity-50"
                        title="Delete goal"
                      >
              ...
              <div className="text-center py-12">
                ...
                <button
                  onClick={() => setShowGoalModal(true)}
                  className="px-6 py-2 bg-gradient-to-r from-primary to-primary text-white rounded-lg font-semibold hover:shadow-lg transition-shadow"
                >
            ...
            >
              ...
              <form onSubmit={handleEditGoal} className="space-y-4">
                ...
                <div className="flex gap-3">
                  ...
                  <button
                    type="submit"
                    className="flex-1 px-4 py-2 bg-gradient-to-r from-primary to-primary text-white rounded-lg"
                  >
                    Update Goal
                  </button>
                </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      ...
                      <div className="text-sm text-muted-foreground">Number of books to complete</div>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <div className="text-sm text-muted-foreground">Total pages to read</div>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <div className="text-sm text-muted-foreground">Reading time in minutes</div>
                    </button>
                    <button
                      type="button"
                    ...
                    >
                      ...
                      <div className="text-sm text-muted-foreground">Consecutive days reading</div>
                    </button>
                  </div>
                </div>
                <div>
        ...
        {toast && (
          <motion.div
            ...
            className={`fixed top-4 right-4 z-50 px-6 py-3 rounded-lg shadow-lg flex items-center gap-3 ${
              toast.type === 'success' ? 'bg-green-50 dark:bg-green-900/200 text-white' : 'bg-red-50 dark:bg-red-900/200 text-white'
            }`}
          >
        ...
        {showCelebration && (
          ...
          >
            <div className="text-center">
              ...
              <h2 className="text-4xl font-bold text-foreground mt-4">Goal Completed! 🎉</h2>
            </div>
          </motion.div>
        )}
      </div>
