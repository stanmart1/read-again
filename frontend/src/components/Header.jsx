  ...
  return (
    <>
      <motion.header 
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        className="bg-card shadow-sm border-b border-border fixed top-0 left-0 right-0 z-50"
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <Link to="/" className="flex items-center space-x-2">
              ...
              <span className="text-xl font-bold font-pacifico text-foreground">
                ReadAgain
              </span>
            </Link>
            ...
            <nav className="hidden md:flex items-center space-x-1">
              {['Home', 'Books', 'Blog', 'FAQ', 'About', 'Contact'].map((item) => (
                <Link
                  ...
                  className="relative px-3 py-2 text-muted-foreground hover:text-primary hover:bg-primary/10 rounded-lg transition-all duration-200 font-medium group"
                >
                  <span>{item}</span>
                  <div className="absolute bottom-0 left-1/2 w-0 h-0.5 bg-primary transition-all duration-200 group-hover:w-4/5 transform -translate-x-1/2"></div>
                </Link>
              ))}
            </nav>
                ...
                <div className="relative">
                  ...
                  {isProfileOpen && (
                    <div className="absolute right-0 mt-2 w-48 bg-card rounded-lg shadow-lg py-2 z-50">
                      ...
                      <Link to="/dashboard" onClick={() => setIsProfileOpen(false)} className="block px-4 py-2 text-foreground hover:bg-muted">
                        <i className="ri-dashboard-line mr-2"></i>Dashboard
                      </Link>
                      <Link to="/dashboard/library" onClick={() => setIsProfileOpen(false)} className="block px-4 py-2 text-foreground hover:bg-muted">
                        <i className="ri-book-line mr-2"></i>My Library
                      </Link>
                      <Link to="/dashboard/orders" onClick={() => setIsProfileOpen(false)} className="block px-4 py-2 text-foreground hover:bg-muted">
      ...
      <motion.div
        ...
        className="fixed right-0 top-0 h-full w-64 bg-card z-[70] md:hidden shadow-lg overflow-y-auto overflow-x-hidden"
      >
        {/* Sidebar Header */}
        ...
        <div className="p-4 space-y-2">
          {['Home', 'Books', 'Blog', 'FAQ', 'About', 'Contact'].map((item) => (
            <Link
              key={item}
              to={item === 'Home' ? '/' : `/${item.toLowerCase()}`}
              className="block px-4 py-3 text-foreground hover:bg-primary/10 hover:text-primary rounded-lg transition-all font-medium"
              onClick={() => setIsMenuOpen(false)}
            >
              {item}
            </Link>
